...
process.stdin.on('end', () => {
  ...
  try {
    ...
    // Detect the always-warm Work Ledger and the continuation policy. When a ledger
    // exists, handoff is a non-event: the agent flushes LEDGER.md at the current task
    // boundary and a fresh session resumes from it via /gsd-resume-work. This supersedes
    // the legacy "do NOT write handoff files, ask the user to pause" advice — the ledger
    // IS the handoff. See get-shit-done/references/work-ledger.md.
    const ledgerPath = path.join(cwd, '.planning', 'LEDGER.md');
    let hasLedger = false;
    let continuationPolicy = 'warm-ledger';
    try {
      const ledgerRaw = fs.readFileSync(ledgerPath, 'utf8');
      hasLedger = true;
      const m = ledgerRaw.match(/continuation_policy:\s*['"]?([a-z-]+)/);
      if (m) continuationPolicy = m[1];
    } catch (e) {
      // No ledger → fall back to legacy STATE.md handoff advice below.
    }
    ...
    // Ledger-aware path (hasLedger): handoff is a non-event — guide the agent to checkpoint
    // the always-warm ledger at a clean boundary so context pressure never catches it
    // mid-task. Legacy path (no ledger): preserve the original STATE.md / pause-work advice.
    let message;
    if (hasLedger) {
      const auto = continuationPolicy === 'autonomous';
      if (isCritical) {
        message =
          `CONTEXT CRITICAL: Usage at ${usedPct}%. Remaining: ${remaining}%. ` +
          'Stop at the LAST CLEAN COMMIT — do not leave a task mid-edit. Make sure LEDGER.md HEAD ' +
          '(Next Action, Current Position, Health) names exactly where to resume, then hand off: a ' +
          'fresh session continues via /gsd-resume-work, reading the ledger first. The ledger IS the ' +
          'handoff — no separate handoff file is needed.' +
          (auto
            ? ' Continuation policy is autonomous: if the working tree is clean and pushed, a ' +
              'fresh-context session may be spawned to continue from the ledger.'
            : '');
      } else {
        message =
          `CONTEXT WARNING: Usage at ${usedPct}%. Remaining: ${remaining}%. ` +
          'Finish the CURRENT task to a clean commit boundary, then flush LEDGER.md (its task record ' +
          'and the HEAD). Avoid starting a new task. Once the ledger is current you can hand off ' +
          'safely at any moment — checkpoint now so context pressure never catches you mid-task.';
      }
    } else if (isCritical) {
      message = isGsdActive
        ? `CONTEXT CRITICAL: Usage at ${usedPct}%. Remaining: ${remaining}%. ` +
