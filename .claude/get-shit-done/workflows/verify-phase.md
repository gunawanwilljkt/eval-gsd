...
<required_reading>
@/Users/gunawanwilljkt/harness/eval-gsd/.claude/get-shit-done/references/verification-patterns.md
@/Users/gunawanwilljkt/harness/eval-gsd/.claude/get-shit-done/templates/verification-report.md
</required_reading>
...
<step name="eval_contract_verdict">
**The merged eval-verify pass — coverage + weakening + gaming + Judge/Human gate rows → ONE
go/no-go verdict recorded in this same VERIFICATION.md.** This absorbs the retired `validate-phase`
(Nyquist) and `eval-review` audits; they no longer run independently. Protocol:
@/Users/gunawanwilljkt/harness/eval-gsd/.claude/get-shit-done/references/eval-first.md
(§3 coverage gate, §4 independence layer, §5 merged verdict, §2.1 lock command).

This section is kept **word-for-word in sync** with `agents/gsd-verifier.md` Step 8b (the file
execute-phase actually spawns) — edit both or they drift.
...
If `EVAL_CONTRACT` is empty → **all of this step is N/A**. Skip every check, record
`### Eval Contract Verdict\n\nN/A — no eval contract for this phase (pre-eval-first / legacy phase).`
and continue with the existing tests-must-pass posture unchanged. **Do NOT block on a missing
contract here** — `eval_first.require_contract` is plan-phase's gate (it already fired before the
phase was planned); double-gating it at verify time would falsely block every legacy phase. Key off
the contract FILE existing, never off the config flag.

If `EVAL_CONTRACT` is non-empty, run the four checks below.

**1. Coverage gate (§3 — REQ ⇄ eval bijection).** Pure function of contract + REQUIREMENTS.md:
- In-scope REQ set = the phase's `REQ-NN` IDs (cross-referenced with `.planning/REQUIREMENTS.md`).
...
Any non-empty `uncovered_reqs` (intent with no eval) or `orphan_rows` (eval with no REQ) → 🛑
**BLOCKER → coverage_hole**.

**2. Weakening detector (§4 — author and verify use the IDENTICAL normalization).** Recompute with
the **exact §2.1 command** that authored the lock (a different normalization would falsely flag/clear
every contract):
...
- (a) `RECOMPUTED != STORED` **and** no logged re-lock decision (an explicit `## Decision` that
  re-locked the hash) → 🛑 **BLOCKER → weakening** (a locked row edited/reordered/deleted/loosened
  without renegotiation).
- (b) Even with a matching hash, each locked `gate`+`Code` row's `command_or_rubric` must still
  appear **verbatim** as a shipped task `<acceptance_criteria>` (`grep -F "$ROW_COMMAND"
  "$PHASE_DIR"/*-PLAN.md`). A locked row dropped / `skip`-ped / loosened → 🛑 **BLOCKER → weakening**.

**3. Gaming detector (§4 — eval bent to the output).** For each `gate` row, find the green commit
(W3's `## LEDGER UPDATE` block in SUMMARY.md carries `commit_sha` + `passing_eval_ids`; also check
`.planning/LEDGER.md`):
...
**flag, do not pass → gaming**. **Honest N/A:** an **inline**-command row (e.g. `python3 -c "…"`,
as the dogfood rows are) has no separate eval file → gaming check is **N/A for that row**.

**4. Gate-row greenness + Judge/Human rows (§5 step 3 — preserve gen ≠ eval).**
- **Code `gate` rows:** green iff their `id` is in the ledger/SUMMARY `passing_eval_ids` evidence.
  **Read the recorded evidence — do NOT re-run the generator's own command** (the per-task HARD GATE
  already ran it; re-running blurs generation≠evaluation). No passing evidence / in `failing_eval_ids`
  → 🛑 **BLOCKER → gate_red**.
- **Judge `gate` rows:** scored **now**, by an agent independent of the code author, against the
  contract's `## Judge Rubrics`. FAIL → 🛑 **BLOCKER → gate_red**.
- **Human `gate` rows:** satisfied via `{NN}-UAT.md`. Not yet PASS → **append it to the
  `identify_human_verification` list** (the single UAT sink) so `determine_status` item 2 routes the
  phase to **human_needed**; do NOT silently pass it.
- **`warn` rows:** reported, **non-blocking**.

**5. Record the merged verdict.** Append an `### Eval Contract Verdict` table to VERIFICATION.md and
set `eval_contract_verdict` in frontmatter. Per §5: green = coverage clean AND weakening clean AND
gaming clean AND every `gate` row green. **Any one failing = the phase is eval-blocked** (feed the
blockers into determine_status below — same posture as "a phase cannot be verified if tests fail").
</step>

<step name="determine_status">
Classify status using this decision tree IN ORDER (most restrictive first):

1. IF any truth FAILED, artifact MISSING/STUB, key link NOT_WIRED, blocker found, **test quality audit found blockers (disabled requirement tests, circular tests), or the eval_contract_verdict step found a coverage hole / weakening / gaming flag / red gate row**:
   → **gaps_found**
...
See /Users/gunawanwilljkt/harness/eval-gsd/.claude/get-shit-done/templates/verification-report.md for complete template.
</step>
...
- [ ] Eval Contract Verdict computed (coverage + weakening §2.1 hash + gaming + Judge/Human gate rows → merged go/no-go) — or recorded N/A when no contract exists
