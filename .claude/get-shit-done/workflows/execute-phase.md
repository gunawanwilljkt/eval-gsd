...
@/Users/gunawanwilljkt/harness/eval-gsd/.claude/get-shit-done/references/agent-contracts.md
@/Users/gunawanwilljkt/harness/eval-gsd/.claude/get-shit-done/references/context-budget.md
...
<ledger_single_writer>
**The orchestrator is the SOLE writer of `.planning/LEDGER.md`** (`references/work-ledger.md` §1).
Executors run in isolated worktrees and only write their own SUMMARY.md (with a `## LEDGER UPDATE`
block per task); they NEVER write the ledger. At each wave boundary the orchestrator parses those
blocks from the merged SUMMARY.md files on disk and writes/commits the ledger in the main tree
(step 5.9). This is additive and backward-compatible: a project with no LEDGER.md gets one seeded
from `templates/ledger.md` on first flush, and the whole flush is best-effort/non-fatal — the legacy
flow still works if the ledger cannot be written.
</ledger_single_writer>
...
- Executor agents omit extended deviation rule examples and checkpoint examples from inline prompt — load on-demand via @/Users/gunawanwilljkt/harness/eval-gsd/.claude/get-shit-done/references/executor-examples.md
- Planner agents omit extended anti-pattern lists and specificity examples from inline prompt — load on-demand via @/Users/gunawanwilljkt/harness/eval-gsd/.claude/get-shit-done/references/planner-antipatterns.md
...
2. For each plan (sequentially, ignoring wave grouping):
   ...
   c. **If "Execute":** Read and follow `/Users/gunawanwilljkt/harness/eval-gsd/.claude/get-shit-done/workflows/execute-plan.md` **inline**
   ...
   **Sequential dispatch for parallel execution (waves with 2+ agents):**
   ...
   Agent(
     ...
     prompt="
       ...
       @/Users/gunawanwilljkt/harness/eval-gsd/.claude/get-shit-done/references/checkpoints.md
       @/Users/gunawanwilljkt/harness/eval-gsd/.claude/get-shit-done/references/tdd.md
       @/Users/gunawanwilljkt/harness/eval-gsd/.claude/get-shit-done/references/worktree-path-safety.md
       ${CONTEXT_WINDOW < 200000 ? '' : '@/Users/gunawanwilljkt/harness/eval-gsd/.claude/get-shit-done/references/executor-examples.md'}
       </execution_context>
...
5.8. **Handle test gate failures (when `WAVE_FAILURE_COUNT > 0`) — BLOCK-OR-ESCALATE, not soft-warn:**

   The post-merge build/test failure is a **hard gate** consistent with loop-control
   (`references/work-ledger.md` §5). Step 5.7 already refuses to mark plans complete when
   `TEST_EXIT≠0`; this step makes the failure **block or escalate** rather than silently
   continuing — a failed wave must NOT advance the phase to "complete". It records the failure
   in the ledger (step 5.9 writes an Open Escalation when `WAVE_FAILURE_COUNT>0`).

   ```
   ## ✗ Post-Merge Build/Test Failure — wave {N} BLOCKED (cumulative failures: ${WAVE_FAILURE_COUNT})

   Wave {N} worktrees merged, but {M} build/test checks fail after merge.
   ...
   **Auto mode** (`AUTO_MODE` / `_auto_chain_active` true — resolve via
   `gsd_run query check auto-mode --pick active`): do **NOT** silently continue. **Escalate +
   pause** (loop-control rung 2): step 5.9 records an Open Escalation in `LEDGER.md` HEAD naming
   the failing wave/tasks, the wave's plans stay `in-progress` (not `done`), and the factory
   **halts** — it does not advance to the next wave or to verification. (The automatic
   retry/attempt-counter ladder S1/S2 is W6; here we record-and-block.)

   **Interactive mode:** present the failure and require an explicit decision before proceeding —
   the failure is already recorded in the ledger, so a fresh session resumes from the escalation:
   ```
   Options:
   1. Fix now (recommended) — resolve conflicts, commit, re-run the gate; only a green gate clears the block
   2. Stop — leave the Open Escalation in the ledger and pause (resume later via /gsd-resume-work)
   ```
   Do NOT offer a "Continue anyway" that marks the wave complete — that is the silent-continue this
   hardening removes. If the user fixes: diagnose (typically import conflicts, missing types, or
   changed signatures from parallel plans touching the same module), fix, commit as
   `fix: resolve post-merge conflicts from wave {N}`, re-run the gate. When the gate goes green
   (`TEST_EXIT=0`, `WAVE_FAILURE_COUNT` not incremented this pass), step 5.7 marks the plans complete
   and step 5.9 clears the escalation + records evidence.

   Note: If `WAVE_FAILURE_COUNT > 1`, compounding failures across waves are exponentially harder to
   diagnose — strongly prefer "Fix now".
   ...
   **Why this matters:** Worktree isolation means each agent's Self-Check passes
   ...
   the next wave builds on a broken foundation — and the ledger record means the block
   survives a context reset (a fresh session sees the Open Escalation and does not blindly retry).

5.9. **Ledger flush (orchestrator is the SOLE writer of `.planning/LEDGER.md`):**

   After the wave's worktrees are merged (step 5.5) and the post-merge gate ran (5.6), the
   orchestrator — **in the main working tree** — writes and commits `.planning/LEDGER.md` from the
   executors' returned status. This is the single-writer invariant (`references/work-ledger.md` §1):
   executors only wrote SUMMARY.md; only the orchestrator writes the ledger.

   **Parse source = the committed/merged SUMMARY.md on disk (authoritative), not the chat return.**
   Each plan's SUMMARY.md carries one `## LEDGER UPDATE` block per task (appended by the executor at
   its self-check site). The chat return may be truncated or never arrive (#2070 / completion-signal
   fallback), so read from disk:

   ```bash
   # Collect every LEDGER UPDATE block from this wave's merged SUMMARY.md files.
   for plan_id in {WAVE_PLAN_IDS}; do
     SUMMARY_FILE="${PHASE_DIR}/${plan_id}-SUMMARY.md"
     [ -f "$SUMMARY_FILE" ] || continue
     # Extract each `## LEDGER UPDATE` block (block runs until the next `## ` heading or EOF).
     awk '/^## LEDGER UPDATE$/{f=1} f{print} /^## /{if(f && !/^## LEDGER UPDATE$/){f=0}}' "$SUMMARY_FILE"
   done
   ```
   This captures each block's `key: value` lines; a following `## ` heading line can leak into the
   output as a trailing line, which is harmless — the parse reads only `key: value` lines and ignores
   any stray heading. (In practice the executor appends LEDGER UPDATE blocks last in SUMMARY.md, so
   they run to EOF with no following heading.) Read the blocks from disk; do not trust the chat echo
   over disk. If a SUMMARY.md is missing its block, reconstruct the minimum — `task`, `status`,
   `commit_sha` from `git log --grep` — with empty eval lists, and flag it; never fabricate
   `passing_eval_ids`.

   **Create-if-absent (backward-compat upgrade path):** if `.planning/LEDGER.md` does not exist, seed
   it before the first flush. **Do NOT `cp` the template file** — `templates/ledger.md` is an
   explainer doc whose real ledger lives inside its ```` ```markdown ```` `## File Template` fenced
   block (raw-cp would write ~165 lines of explainer as the project ledger). Instead, the same way GSD
   instantiates `summary.md` / `eval-contract.md`: **Write** `.planning/LEDGER.md` from that fenced
   `## File Template` block — frontmatter `ledger_version:'1.0'`, `status: in-progress`,
   `generation: 1`, `forward_progress: 0`, `continuation_policy` from config, `position` (this
   phase/plan/task), `sole_writer: orchestrator` — plus the Tier-1 HEAD (`Next Action`,
   `Current Position`, empty `Open Escalations`, `Health`). Subsequent flushes edit this instantiated
   file. This is best-effort and **non-fatal** — a seed/write failure logs a warning and skips the
   flush; it must not break a legacy project's wave.

   **Merge each block into the ledger (gated on the SAME `TEST_EXIT` as step 5.7):**

   For every `## LEDGER UPDATE` block, write/update that task's **Task Record** (Tier 2) per
   `templates/ledger.md`:
   - `status` → task record `status`
   - `commit_sha` → `evidence.commit_sha`
   - `passing_eval_ids` → `evidence.passing_eval_ids`; `failing_eval_ids` → the task's red rows
   - `self_check: FAILED` → the task is **NOT** `done` regardless of its `status` line (git-truth, §3)
   - `blocker` → task record `blocker`; append an `attempts[]` entry from `attempt_summary`

   Then **gate the HEAD recompute on the post-merge result**, mirroring 5.7:
   - **`TEST_EXIT=0` AND `WAVE_FAILURE_COUNT==0`:** mark this wave's tasks `done` with their evidence,
     recompute the Tier-1 HEAD (`Next Action`, `Current Position`, `Health`), advance position.
   - **`TEST_EXIT≠0` OR `WAVE_FAILURE_COUNT>0`:** do **NOT** mark the wave's plans `done`. Record an
     **Open Escalation** in the HEAD `## Open Escalations` (format: `templates/ledger.md` / work-ledger
     §6) naming the failing wave + tasks + the failing checks, set `status: blocked` on the affected
     tasks, and leave `Next Action` pointing at the fix. This is what makes the hardened 5.8 gate
     survive a reset: a fresh session reads the escalation and pauses (does not blindly retry).

   **Commit the ledger separately (two-commit convention, work-ledger §2):** the executors' per-task
   **work** commits already landed (merged in 5.5); the orchestrator commits ONLY the ledger here, so
   `git log` shows the ledger was written exclusively by the orchestrator and `git show <work_sha>`
   stays clean for the gaming check:
   ```bash
   if [ -f .planning/LEDGER.md ] && ! git diff --quiet .planning/LEDGER.md 2>/dev/null; then
     gsd_run query commit "chore(ledger): flush wave ${N} task records + HEAD" --files .planning/LEDGER.md \
       || echo "⚠ Ledger flush commit skipped (non-fatal) — recompute on next wave"
   fi
   ```
   Recompute the HEAD on **every** wave boundary so it is never more than one wave stale. The flush is
   best-effort/non-fatal end-to-end: any failure here logs a warning and continues — it must not break
   the wave (backward-compat). The ledger commit is the ONLY place `LEDGER.md` is written in the entire
   execute path; no executor and no other step writes it.
...
**Ledger escalation guard (block-or-escalate, before any completion claim):**
If `.planning/LEDGER.md` exists, read its Tier-1 HEAD `## Open Escalations`. If it is non-empty
(a wave failed its post-merge gate in step 5.8/5.9), the factory is **PAUSED** on those tasks —
do **NOT** mark the phase complete, do **NOT** proceed to verification. Surface the escalation(s)
and stop:
```
## Phase {X} BLOCKED — open ledger escalation(s)
{render each ## Open Escalations entry: task, failing checks, the decision needed}
Resolve, then re-run /gsd-execute-phase {phase} (or /gsd-resume-work to continue from the ledger).
```
If `## Open Escalations` is empty (or no LEDGER.md — legacy project), continue normally.
...
Read and follow `/Users/gunawanwilljkt/harness/eval-gsd/.claude/get-shit-done/workflows/transition.md`, passing through the `--auto` flag so it propagates to the next phase invocation.
