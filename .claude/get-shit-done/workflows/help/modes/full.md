...
- Saves timestamped note to `.planning/notes/` (or `/Users/gunawanwilljkt/harness/eval-gsd/.claude/notes/` globally)
...
- **`/gsd-validate-phase [phase]`** — *(retired — merged into `/gsd-verify-work`)* Nyquist validation is now part of the merged eval verdict; this alias no longer runs an independent audit.
- **`/gsd-ui-review [phase]`** — Retroactive 6-pillar visual audit of implemented frontend code.
- **`/gsd-eval-review [phase]`** — *(retired — merged into `/gsd-verify-work`)* AI eval-coverage auditing is now part of the merged eval verdict; this alias no longer runs an independent audit.
...
├── LEDGER.md             # Two-tier work ledger (HEAD + task history) for cross-session resume
...
## Eval-First & the Work Ledger

**Eval-first.** When `eval_first.require_contract` is on, an **eval contract**
(`.planning/phases/NN-name/NN-EVAL-CONTRACT.md`) is authored and **locked at spec
time** (`/gsd-spec-phase`, or `/gsd-discuss-phase` as a fallback). It maps each in-scope
requirement to an acceptance check, so `/gsd-plan-phase` refuses to plan without a locked
contract, `/gsd-execute-phase` hard-gates each task on its acceptance command, and
`/gsd-verify-work` produces one merged go/no-go (coverage + weakening + gaming + Judge/Human).
This absorbs the retired `/gsd-validate-phase` and `/gsd-eval-review` audits.

**The Work Ledger** (`.planning/LEDGER.md`) is a two-tier, git-versioned task list —
an always-read HEAD plus append-only history. `/gsd-execute-phase` is its sole writer, and
`/gsd-resume-work` reads its HEAD first and git-verifies prior commits, so a fresh session
(or an autonomous handoff) picks up exactly where the last one stopped.
