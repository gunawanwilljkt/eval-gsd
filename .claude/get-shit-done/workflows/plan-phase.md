...
@/Users/gunawanwilljkt/harness/eval-gsd/.claude/get-shit-done/references/ui-brand.md
@/Users/gunawanwilljkt/harness/eval-gsd/.claude/get-shit-done/references/revision-loop.md
@/Users/gunawanwilljkt/harness/eval-gsd/.claude/get-shit-done/references/gate-prompts.md
...
When `TDD_MODE` is `true`, the planner agent is instructed to apply `type: tdd` to eligible tasks using heuristics from `references/tdd.md`. The planner's `<required_reading>` is extended to include `@/Users/gunawanwilljkt/harness/eval-gsd/.claude/get-shit-done/references/tdd.md` so gate enforcement rules are available during planning.
...
## 1.6. Eval-First Contract Gate (eval_first.require_contract)

> The eval-first protocol authors and **locks** the phase's eval contract at spec/discuss time
> (`workflows/spec-phase.md` Step 6.5, or `workflows/discuss-phase.md` `author_eval_contract`
> fallback). plan-phase refuses to proceed when that contract is missing or still `draft` — so
> evals are pre-committed *before* the plan that will be gated on them. The lock procedure +
> normalization are defined once in
> `@/Users/gunawanwilljkt/harness/eval-gsd/.claude/get-shit-done/references/eval-first.md` §2.1.
> **Additive / backward-compatible:** if `eval_first.require_contract` is absent or `false`, this
> gate is a no-op and plan-phase behaves exactly as it did before.

**Skip this gate for runs that produce no plan.** `--research-phase <N>` (research-only) and
`--view` (print existing RESEARCH.md) route through this same command but generate no PLAN.md, so
gating them would regress those flows (and for `--research-phase N` the phase number is the flag
value, not yet resolved into `$PHASE` at this point — §2 parses it). Mirror §1.5's inline-args
style:

```bash
if [[ "$ARGUMENTS" =~ --research-phase ]] || [[ "$ARGUMENTS" =~ (^|[[:space:]])--view([[:space:]]|$) ]]; then
  : # research-only / view-only — no plan produced, nothing to gate; continue to §2
else
  REQUIRE_CONTRACT=$(gsd_run query config-get eval_first.require_contract 2>/dev/null || echo "false")
fi
```

**If the run is research-only/view-only, or `REQUIRE_CONTRACT` is not `true`:** skip this gate
entirely (no-op). Continue to §2.

**If `REQUIRE_CONTRACT` is `true`:** the phase must have a **`status: locked`** eval contract.
Locate it and read its status (`phase_dir` is resolved in §1 init; if the phase dir does not
exist yet there can be no contract, which correctly trips the gate):

```bash
EVAL_CONTRACT=$(ls "${phase_dir}"/*-EVAL-CONTRACT.md 2>/dev/null | head -1 || true)
CONTRACT_STATUS=""
if [ -n "$EVAL_CONTRACT" ]; then
  # status is the frontmatter `status:` line (draft | locked)
  CONTRACT_STATUS=$(grep -m1 -E '^status:[[:space:]]*' "$EVAL_CONTRACT" | sed -E 's/^status:[[:space:]]*//; s/[[:space:]].*$//' | tr -d "'\"")
fi
```

**Decision (block unless the contract exists AND is `locked`):**

- **`EVAL_CONTRACT` empty (no contract file)** → **BLOCK.** Emit and exit:
  ```
  Phase ${PHASE} has no eval contract, but eval_first.require_contract is true.
  Author + lock it before planning:

    /gsd-spec-phase ${PHASE}      (primary — writes SPEC.md + the eval contract)
    /gsd-discuss-phase ${PHASE}   (fallback — authors the contract during context capture)

  To plan without an eval contract, set eval_first.require_contract: false in
  .planning/config.json (not recommended — disables the eval-first gate).
  ```
- **`CONTRACT_STATUS` is `draft` (or anything other than `locked`)** → **BLOCK.** Emit and exit:
  ```
  Phase ${PHASE} eval contract is '${CONTRACT_STATUS:-unknown}', not 'locked'.
  A draft contract failed its coverage gate (uncovered REQs or orphan rows) — it cannot gate
  execution until it locks. Open ${EVAL_CONTRACT}, fix the coverage holes listed in its
  `coverage:` frontmatter, then re-run /gsd-spec-phase ${PHASE} (or /gsd-discuss-phase ${PHASE})
  to re-lock it. See references/eval-first.md §2.1–§3.
  ```
- **`CONTRACT_STATUS` is `locked`** → **PASS.** Continue to §2. (Backstop coverage check:
  `verify-phase` recomputes `locked_hash` and re-runs the coverage gate; plan-phase trusts the
  `locked` status set at author time.)

This gate is a hard stop, mirroring §1.5's posture — exit the workflow on BLOCK; do not continue
to argument parsing, research, or the planner spawn.
...
- Planner is instructed to produce `SKELETON.md` in the phase directory alongside `PLAN.md`. The template lives at `@/Users/gunawanwilljkt/harness/eval-gsd/.claude/get-shit-done/references/skeleton-template.md`.
...
**If found:**
1. Read template: `/Users/gunawanwilljkt/harness/eval-gsd/.claude/get-shit-done/templates/VALIDATION.md`
...
${TDD_MODE === 'true' ? `
<tdd_mode_active>
**TDD Mode is ENABLED.** Apply TDD heuristics from @/Users/gunawanwilljkt/harness/eval-gsd/.claude/get-shit-done/references/tdd.md to all eligible tasks:
...
**MVP_MODE:** ${MVP_MODE} (when true, follow vertical-slice rules from `@/Users/gunawanwilljkt/harness/eval-gsd/.claude/get-shit-done/references/planner-mvp-mode.md`; when false, ignore MVP guidance entirely.)
...
${MVP_MODE === 'true' ? `
<mvp_mode_active>
**MVP Mode is ENABLED.** Follow vertical-slice planning rules from @/Users/gunawanwilljkt/harness/eval-gsd/.claude/get-shit-done/references/planner-mvp-mode.md. Each plan must deliver a complete vertical slice — thin end-to-end functionality rather than horizontal layers.
</mvp_mode_active>
` : ''}
</planning_context>
...
if [ "$POST_PLANNING_GAPS" = "true" ]; then
  node "/Users/gunawanwilljkt/harness/eval-gsd/.claude/get-shit-done/bin/gsd-tools.cjs" gap-analysis --phase-dir "${PHASE_DIR}"
fi
```
