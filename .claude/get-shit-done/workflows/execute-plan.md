...
@/Users/gunawanwilljkt/harness/eval-gsd/.claude/get-shit-done/references/git-integration.md
</required_reading>
...
3. **Critical ordering — write and commit SUMMARY.md as one atomic block.** Do NOT
   ...
   - Append one `## LEDGER UPDATE` block per task after the `## Self-Check:` line (see
     `<step name="ledger_update_return">`) — do NOT write/commit `LEDGER.md` (orchestrator is sole writer)
...
See `/Users/gunawanwilljkt/harness/eval-gsd/.claude/get-shit-done/references/tdd.md` for structure.
</tdd_plan_execution>
...
See /Users/gunawanwilljkt/harness/eval-gsd/.claude/get-shit-done/references/checkpoints.md for details.
</step>
...
If user_setup exists: create `{phase}-USER-SETUP.md` using template `/Users/gunawanwilljkt/harness/eval-gsd/.claude/get-shit-done/templates/user-setup.md`. Per service: env vars table, account setup checklist, dashboard config, local dev notes, verification commands. Status "Incomplete". Set `USER_SETUP_CREATED=true`. If empty/missing: skip.
</step>
...
Create `{phase}-{plan}-SUMMARY.md` at `.planning/phases/XX-name/`. Use `/Users/gunawanwilljkt/harness/eval-gsd/.claude/get-shit-done/templates/summary.md`.
...
**After the `## Self-Check:` line, append one `## LEDGER UPDATE` block per task** (see
`<ledger_update_return>` below) into this same SUMMARY.md. NEVER write or commit `LEDGER.md` here —
the execute-phase orchestrator (main tree) is the single writer (`references/work-ledger.md` §1).
</step>

<step name="ledger_update_return">
Append a `## LEDGER UPDATE` block **per task** to SUMMARY.md, right after the `## Self-Check:` line.
This is the structured status the execute-phase orchestrator merges into `.planning/LEDGER.md`.

**Single-writer invariant (non-negotiable):** the orchestrator in the **main working tree** is the
ONLY writer of `LEDGER.md`. You run in an isolated worktree — writing the ledger here would diverge
parallel worktree copies and corrupt on merge. You write SUMMARY.md only and **return** status; the
orchestrator parses the committed/merged SUMMARY.md on disk at the wave boundary (the durable channel
— the chat return may truncate or never arrive; see execute-phase.md #2070 / completion-signal
fallback). Schema (verbatim from `references/work-ledger.md` §1 — identical field names/order):
...
passing_eval_ids: [EC-..]
failing_eval_ids: [EC-..]
attempt_summary: "<one line: what was built + acceptance_criteria result>"
self_check: PASSED | FAILED
blocker: none | "<one line>"
```

Populate from data already in hand (no new computation):
- `status`: `done` if the task's `<acceptance_criteria>` HARD GATE cleared; `red` if a criterion
  stayed red after the gate's fix attempts; `blocked` on a checkpoint/auth gate or hard blocker.
- `commit_sha`: the per-task **work** commit short sha (not the docs/metadata commit — the gaming
  check `git show <sha> --stat` keys on it). `none` if no commit.
- `passing_eval_ids`/`failing_eval_ids`: the task's `eval_rows: [EC-..]` (carried by the planner, W1),
  split by the acceptance-gate result. **No contract / no `eval_rows` → both `[]`** (block still emits).
- `self_check`: mirror the plan's `## Self-Check:` verdict.

Purely additive: appends text to SUMMARY.md. Legacy projects (no LEDGER.md, no contract) are
unaffected — the orchestrator create-if-absent's the ledger from the template and reads empty lists.
</step>
...
- `## LEDGER UPDATE` block appended to SUMMARY.md per task (per `<step name="ledger_update_return">`); `LEDGER.md` itself NOT written/committed here (execute-phase orchestrator is sole writer)
