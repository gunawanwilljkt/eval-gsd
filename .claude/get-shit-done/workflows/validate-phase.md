<deprecated>
**RETIRED — merged into verify-phase (eval-first, W5).** `validate-phase` no longer runs an
independent Nyquist audit. The eval contract authored at spec time (`{NN}-EVAL-CONTRACT.md`) is the
single source of "what proves this phase", and `verify-phase`'s **Eval Contract Verdict** step
(coverage + weakening + gaming + Judge/Human gate rows → one go/no-go) is the single place it is
checked. Nyquist's useful part — sampling cadence — lives in the contract's `sample_rate`. See
...
`gsd-nyquist-auditor` or run any of the steps that follow — they are kept only for reference / the
backward-compat fallback noted next.

```
GSD > /gsd-validate-phase is RETIRED — eval coverage is now verified by /gsd-verify-work
      (verify-phase's merged Eval Contract Verdict). Re-run /gsd-verify-work {N} instead.
```

**Backward-compat fallback (only if `eval_first.require_contract` is false AND the phase has no
`{NN}-EVAL-CONTRACT.md`):** a legacy project that never adopted eval-first has nothing for
verify-phase to check, so the original Nyquist flow below MAY run as a one-off. With eval-first on
(the default) this never fires — verify-phase owns the verdict.
</deprecated>
...
```
Agent(
  prompt="Read /Users/gunawanwilljkt/harness/eval-gsd/.claude/agents/gsd-nyquist-auditor.md for instructions.\n\n" +
...
**State B (create):**
1. Read template from `/Users/gunawanwilljkt/harness/eval-gsd/.claude/get-shit-done/templates/VALIDATION.md`
