...
Also read `get-shit-done/references/work-ledger.md` — this workflow is the autonomous factory's
top-level orchestrator and applies that reference's **§4 loop-control signals (S1/S2)**, **§5
escalation ladder + forward-progress guard**, and **§6 Open Escalation** format at task/phase
boundaries.
...
**Writer boundaries (the single-writer invariant is subagent-vs-orchestrator, not one-orchestrator —
`references/work-ledger.md` §1).** Both `execute-phase` and this workflow run in the **main working
tree**, so both may write `.planning/LEDGER.md`; they never write it concurrently (autonomous calls
execute-phase synchronously and acts only at its boundaries). To avoid double-writes, the field
ownership is split:

| Field / artifact | Sole writer | When |
|---|---|---|
| Task Records (`status`, `evidence`, `attempts[]`, `green_eval_count`) | **execute-phase** (W3, step 5.9) | per task/wave |
| `## Open Escalations` from a **wave gate** failure | **execute-phase** (W3, step 5.9) | per wave |
| `generation`, `forward_progress` | **this workflow** (takeover guard) | once per generation, at resume/takeover |
| `## Open Escalations` from a **loop-control S1/S2** trip + factory pause | **this workflow** | at phase-boundary routing |
| Tier-1 HEAD recompute at a **phase** boundary | **this workflow** | each phase boundary |

This workflow is the autonomous loop's top orchestrator: it owns the generation/forward-progress
guard (§5), the S1/S2 loop-control ladder (§4), and the phase-boundary HEAD flush. It NEVER edits a
Task Record's per-task fields (execute-phase owns those) and it NEVER writes the ledger from a
subagent. All edits are additive/backward-compatible: **if `.planning/LEDGER.md` does not exist
(legacy / non-ledgered project), every ledger step below is a silent no-op** and autonomous behaves
exactly as before.
...
**1b. Generation takeover — forward-progress guard (`references/work-ledger.md` §5).**

Each `/gsd-autonomous` invocation on an **in-progress** milestone is a new generation taking over.
Apply the §5 guard ONCE here, before running any phase — it is the anti-respawn check that makes the
autonomous loop provably terminating. **If `.planning/LEDGER.md` does not exist, skip this entire
sub-step** (legacy project — behave exactly as before).

```bash
LEDGER=.planning/LEDGER.md
if [ -f "$LEDGER" ]; then
  # 1. First honor any pre-existing pause: a non-empty `## Open Escalations` means the factory is
  #    already paused on a human decision — do NOT take over / bump / run a phase.
  OPEN_ESC=$(awk '/^## Open Escalations$/{f=1;next} /^## /{f=0} f && $0 !~ /^None\.?$/ && $0 ~ /[^[:space:]]/{print}' "$LEDGER")
  # 2. C_now = completed-task count = git-verified `done` task records (the just-ended gen's output).
  #    INHERITED = the inherited forward_progress (the just-ended gen's STARTING baseline).
  INHERITED=$(grep -E '^forward_progress:' "$LEDGER" | head -1 | sed -E 's/^forward_progress:[[:space:]]*//; s/[[:space:]]*#.*$//')
  GEN=$(grep -E '^generation:' "$LEDGER" | head -1 | sed -E 's/^generation:[[:space:]]*//; s/[[:space:]]*#.*$//')
  LSTATUS=$(grep -E '^status:' "$LEDGER" | head -1 | sed -E 's/^status:[[:space:]]*//; s/[[:space:]]*#.*$//')
fi
```

Then, **only if the ledger exists**, run the §5 takeover guard (this workflow is the SOLE writer of
`generation` + `forward_progress`):

1. **If `$OPEN_ESC` is non-empty** → the factory is already PAUSED. Surface the escalation block
   verbatim and **STOP** (do not bump generation, do not run the guard, do not start a phase). This
   is the same PAUSE the loop-control ladder (step 3d) and `/gsd-resume-work` produce.
2. **If `$LSTATUS` is `complete`** (terminal) → nothing to take over: do not bump or guard; fall
   through to discover_phases (which will report "all phases complete").
3. **Otherwise (real work remains): run the guard, then bump + snapshot.** Compute `C_now` =
   the count of `done` Task Records whose `evidence.commit_sha` git-verifies (`git cat-file -e
   <sha>^{commit}` + `git merge-base --is-ancestor <sha> HEAD`). Then:
   - **If `C_now == $INHERITED` AND incomplete tasks remain** → the entire just-ended generation
     produced **zero** net new completed tasks. Do NOT respawn. Write an **Open Escalation** (§6
     format, see step 3c.7's `write_escalation`) naming the stalled phase/task + "no forward progress
     this generation (gen `$GEN`)", set the ledger frontmatter `status: blocked`, recompute HEAD,
     commit the ledger (`chore(ledger): forward-progress guard halt — gen ${GEN}`), and **STOP the
     factory** (route to handle_blocker with "no forward progress — paused, awaiting human").
   - **Else** → set `generation := $GEN + 1`, `forward_progress := C_now` (this generation's
     starting baseline), recompute the Tier-1 HEAD, commit
     (`chore(ledger): generation takeover — gen ${NEW_GEN}, fp=${C_now}`), and proceed to
     discover_phases. The per-task execute-phase flush NEVER touches these two fields, so there is
     no double-count.

> **Seeding note (`references/work-ledger.md` §5):** the run that *creates* the ledger seeds
> `generation: 1, forward_progress: 0` and does NOT guard (no prior generation). Only takeovers
> (resuming an existing in-progress ledger) reach the guard. A genuinely stuck gen-1 is caught at the
> gen-2 takeover (`C_now == 0 == forward_progress`) — there is no `C > 0` exemption.
...
**3c.7. Loop-control ladder (S1/S2 → rung-1 retry / rung-2 escalate + pause)**

> `references/work-ledger.md` §4 (signals) + §5 (ladder). **If `.planning/LEDGER.md` does not exist,
> skip this entire sub-step** (legacy project — route on VERIFICATION.md alone, exactly as before).

Loop-control is **derived purely from the ledger task records** (no conversation memory), so a fresh
session reconstructs "this task already failed N times" with zero prior context. After execute-phase
returns it has already flushed each task's record (W3, step 5.9) including any `attempts[]` entry and
the `## Open Escalations` block. Read those and decide whether to retry, escalate, or continue.

```bash
LEDGER=.planning/LEDGER.md
if [ -f "$LEDGER" ]; then
  # Wave-gate failures already left an Open Escalation (W3 step 5.9) — honor it first.
  OPEN_ESC=$(awk '/^## Open Escalations$/{f=1;next} /^## /{f=0} f && $0 !~ /^None\.?$/ && $0 ~ /[^[:space:]]/{print}' "$LEDGER")
fi
```

Apply the ladder, in order (only when the ledger exists):

1. **If `$OPEN_ESC` is non-empty** → execute-phase already recorded a rung-2 escalation (a wave gate
   failed and it paused). **Do not retry blindly** (§9 anti-pattern). Surface the escalation verbatim
   and route to **handle_blocker** with the escalation's `THE DECISION I NEED` — the factory is paused
   until a human answers. STOP autonomous iteration for this phase.

2. **Otherwise, evaluate S1/S2 per task** for any task in this phase whose latest attempt is red
   (`status: blocked`, or a `done`-but-`Self-Check: FAILED`, or a non-empty `failing_eval_ids`). For
   each such task, count its `attempts[]` entries (Tier-2 Task Record):
   - **`attempts >= 5` → S2 (lifetime cap) trips → rung 2** (below). Hard cap; do not retry further.
   - **`consecutive red attempts >= 3` → S1 trips → rung 2** (below). (Consecutive: a task whose
     `green_eval_count` rose between attempts never trips S1 — the failures must be unbroken.)
   - **`consecutive red attempts < 3` AND `attempts < 5` → rung 1 (retry with a different approach):**
     re-attempt the failing task, **inlining the failing eval IDs + a one-line "what was tried"**
     (from the last `attempts[]` entry) so the retry does NOT repeat the same approach. In autonomous
     mode the retry is the existing gap-closure path:
     ```
     Skill(skill="gsd-plan-phase", args="${PHASE_NUM} --gaps")
     Skill(skill="gsd-execute-phase", args="${PHASE_NUM} --no-transition")
     ```
     execute-phase appends a new `attempts[]` entry on its flush, so the counter advances and the next
     pass re-reads S1/S2 from the ledger — the retry budget is the ledger's, not this loop's memory.
     This **replaces** the old open-ended "Run gap closure" prompt with a bounded, ledger-counted
     retry: at most 2 rung-1 retries (attempts 1→2→3) before S1 forces rung 2.

3. **Rung 2 — escalate to human + pause the factory** (S1 tripped, or S2 cap hit, or a hard blocker).
   Run `write_escalation` (below) for the offending task, then route to **handle_blocker** with the
   escalation question and STOP — the factory is paused; a fresh session resumes from the Open
   Escalation via `/gsd-resume-work` and does not blindly retry (§9). Do NOT continue to the iterate
   step while an Open Escalation is open.

**`write_escalation` — Write an Open Escalation (rung 2), `references/work-ledger.md` §6 format.**
This workflow is the
writer of loop-control escalations (the field-ownership table in `<ledger_orchestration>`). Append to
the ledger HEAD `## Open Escalations` block (replacing the `None.` placeholder if present), set the
task `escalation_rung: 2` + `status: blocked` and the frontmatter `status: blocked`, recompute the
Tier-1 HEAD, and commit ONLY the ledger (`chore(ledger): rung-2 escalation — <task> (<signal>)`):

```
### ESCALATION — task {phase-plan-task} (rung 2)
Intent at stake: {REQ-NN} — "{the requirement text}"
Converged / didn't: {green EC ids} green; {red EC ids} red after {N} attempts
Why I'm stuck: {diagnosis — bad eval / impossible task / spec conflict, from the last attempt}
Evidence: commits [{shas}]; failing eval cmds [{the failing acceptance_criteria commands}]
THE DECISION I NEED (pick one):
  [a] eval contract wrong — relax {EC-id} to {X}
  [b] re-scope task to {Y}
  [c] accept current state and defer
  [d] other
```

The signal name is `S1 (3 consecutive eval-gate failures)` or `S2 (lifetime cap 5 attempts)` or
`hard blocker`. The block is self-contained: a human (or a fresh session) answers without
re-deriving context. On the answer, the normal flow records the decision in the Decision Log, clears
the escalation, sets the task back to `doing` at the chosen path, and resumes (§6).
...
**Ledger-governed path (when `.planning/LEDGER.md` exists):** the loop-control ladder (step 3c.7)
already ran on the flushed task records and made the bounded decision — a rung-1 retry (gap closure,
budget counted in `attempts[]`) or a rung-2 escalation + pause (Open Escalation written, factory
halted). Do NOT additionally ask the open-ended question below; that would double-count the retry
budget and re-introduce the unbounded thrash the ladder removes. If 3c.7 escalated → you are already
at handle_blocker. If 3c.7 retried and the re-execute is now green/human_needed → route normally. If
it retried and gaps still persist, the next 3c.7 pass advances `attempts[]` toward the S1/S2 trip.

**Legacy fallback (no ledger):** keep the original interactive routing — ask user via
AskUserQuestion:
...
Read and execute: `/Users/gunawanwilljkt/harness/eval-gsd/.claude/get-shit-done/references/autonomous-smart-discuss.md`
...
**Otherwise:** After each phase completes — this is a **phase boundary** — flush the ledger HEAD,
then re-read ROADMAP.md to catch phases inserted mid-execution (decimal phases like 5.1).

**Phase-boundary ledger HEAD flush (`references/work-ledger.md` §1; skip if no `.planning/LEDGER.md`).**
execute-phase already flushed each task's record at its wave boundaries (W3, step 5.9); at this phase
boundary the autonomous orchestrator only **recomputes the Tier-1 HEAD** so a fresh session resuming
mid-milestone reads the correct `Next Action` / `Current Position` / `Health` with no derivation:
- `## Current Position` → the next incomplete phase / "Phase N complete";
- `## Next Action` → "discuss + plan + execute phase {next_incomplete}" (or closeout if none remain);
- `## Health` → `done/total` tasks, last few outcomes, any stuck-watch.
This recompute touches only the HEAD (not Task Records, not `generation`/`forward_progress`). If the
HEAD changed, commit ONLY the ledger (`chore(ledger): phase {N} boundary — recompute HEAD`). It is
best-effort/non-fatal — a failure logs a warning and continues the milestone (backward-compat).
...
- [ ] When `.planning/LEDGER.md` exists, a takeover runs the §5 forward-progress guard once (step 1b): `C_now == forward_progress` + work remaining → halt + escalate; else bump generation + snapshot `forward_progress := C_now`
- [ ] This workflow is the SOLE writer of `generation` + `forward_progress` (per-task execute-phase flush never touches them) — no double-count
- [ ] Loop-control ladder (step 3c.7) derives S1 (≥3 consecutive eval-gate failures) + S2 (lifetime cap 5 attempts) purely from ledger task records' `attempts[]` (no conversation memory)
- [ ] Rung 1 (attempts < 3, < 5) retries with a different approach, inlining the failing eval IDs; rung 2 (S1/S2/hard-blocker) writes a §6 Open Escalation + sets task `status: blocked` + pauses the factory
- [ ] An unsatisfiable task escalates after 3 attempts and pauses — it does not thrash (bounded retry budget counted in the ledger, not in this loop's memory)
- [ ] A pre-existing non-empty `## Open Escalations` is honored (PAUSE) before bumping/guard/retry — no blind retry of a rung-2 task (§9)
- [ ] Phase-boundary HEAD flush recomputes the Tier-1 HEAD (Next Action / Current Position / Health) at each phase boundary
- [ ] All ledger steps are no-ops when `.planning/LEDGER.md` is absent (legacy/non-ledgered projects unaffected)
</success_criteria>
