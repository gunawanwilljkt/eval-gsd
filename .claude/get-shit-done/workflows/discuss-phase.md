...
<required_reading>
@/Users/gunawanwilljkt/harness/eval-gsd/.claude/get-shit-done/references/domain-probes.md
@/Users/gunawanwilljkt/harness/eval-gsd/.claude/get-shit-done/references/gate-prompts.md
@/Users/gunawanwilljkt/harness/eval-gsd/.claude/get-shit-done/references/universal-anti-patterns.md
...
if [ -f "/Users/gunawanwilljkt/harness/eval-gsd/.claude/get-shit-done/USER-PROFILE.md" ]; then
  ADVISOR_MODE=true
else
  ADVISOR_MODE=false
fi
...
Read `@/Users/gunawanwilljkt/harness/eval-gsd/.claude/get-shit-done/references/scout-codebase.md` — it contains the phase-type→map selection table, single-read rule, no-maps fallback, and `<codebase_context>` output schema. Then execute:
...
<step name="author_eval_contract">
**Eval-first fallback (only when spec-phase was skipped).** spec-phase is the primary home for
authoring + locking the phase's eval contract; this step is the fallback so a project that goes
straight discuss→plan still produces a `locked` contract — otherwise plan-phase's eval-first
gate (active when `eval_first.require_contract` is true, the template default) would block with
no way to satisfy it.

```bash
REQUIRE_CONTRACT=$(gsd_run query config-get eval_first.require_contract 2>/dev/null || echo "false")
EXISTING_CONTRACT=$(ls "${phase_dir}/${padded_phase}-EVAL-CONTRACT.md" 2>/dev/null || true)
EXISTING_STATUS=""
if [ -n "$EXISTING_CONTRACT" ]; then
  EXISTING_STATUS=$(grep -m1 -E '^status:[[:space:]]*' "$EXISTING_CONTRACT" | sed -E 's/^status:[[:space:]]*//; s/[[:space:]].*$//' | tr -d "'\"")
fi
```

**Skip this step if** `REQUIRE_CONTRACT` is not `true` (behave exactly as before), **or** if a
contract already exists AND its `EXISTING_STATUS` is `locked` (spec-phase, or an earlier run,
already authored + locked it — do not overwrite a locked contract).

**Do NOT skip on a `draft` contract** — re-run the coverage gate and re-attempt the lock on it.
A `draft` means a previous run hit coverage holes; skipping it would leave a discuss-only project
permanently unable to lock (and would make plan-phase §1.6's "re-run /gsd-discuss-phase" remedy a
no-op). Re-author/repair the rows, re-run the gate, lock when clean.

**Otherwise**, read `@/Users/gunawanwilljkt/harness/eval-gsd/.claude/get-shit-done/references/eval-first.md`
§2.1 and execute its **authoring & lock procedure** verbatim — the *same* procedure spec-phase
uses, so the lock is identical and reproducible by verify-phase:

1. **In-scope REQ set** — the phase's `REQ-NN` IDs (the IDs plan-phase consumes as
   `phase_req_ids`), drawn from `<locked_requirements>` (if SPEC.md was loaded) or by reading
   `.planning/REQUIREMENTS.md` and selecting the entries scoped to this phase (`init.phase-op`
   does not expose `phase_req_ids`):
   ```bash
   cat .planning/REQUIREMENTS.md 2>/dev/null || true   # select the REQ-NN ids in this phase's scope
   ```
2. **Author** `${phase_dir}/${padded_phase}-EVAL-CONTRACT.md` from
   ...
   one behavioral-acceptance `## Rows` entry per provable claim, each tagged with a real
   in-scope `req_ref`. Set `status: draft`, `coverage.requirements` to the in-scope REQ set.
3. **Coverage gate (§3):** fill `coverage.uncovered_reqs` / `coverage.orphan_rows` / `rows_total`.
4. **Lock or stay draft:**
   - **Both arrays empty →** set `status: locked`, `locked_at`, and compute `locked_hash`:
     ```bash
   ...
   - **Either array non-empty →** leave `status: draft`, do **not** set `locked_hash`, and tell
     the user which REQs need a row / which rows are orphaned. plan-phase will block until it
     locks; fix and re-run `/gsd-discuss-phase ${PHASE}` (or author the contract in
     `/gsd-spec-phase ${PHASE}`).

Add the contract to the `git_commit` step's `--files` list so it commits with CONTEXT.md.
</step>
...
Commit phase context and discussion log (include the eval contract when the
`author_eval_contract` step wrote one):
```bash
COMMIT_FILES=("${phase_dir}/${padded_phase}-CONTEXT.md" "${phase_dir}/${padded_phase}-DISCUSSION-LOG.md")
[ -f "${phase_dir}/${padded_phase}-EVAL-CONTRACT.md" ] && COMMIT_FILES+=("${phase_dir}/${padded_phase}-EVAL-CONTRACT.md")
...
- Eval-first fallback: when `eval_first.require_contract` is true AND spec-phase did not already
  author one, `{padded_phase}-EVAL-CONTRACT.md` is authored, coverage-gated, and `locked` (or
  left `draft` with the user told which REQs/rows to fix), via the single `references/eval-first.md`
  §2.1 procedure — and committed alongside CONTEXT.md
