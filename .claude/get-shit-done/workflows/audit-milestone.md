...
Discovery only — never auto-calls the verifier (`/gsd-verify-work`, which absorbed the retired `/gsd-validate-phase`).
...
|-------|---------------|-----------|--------|
| {phase} | exists/missing | true/false/partial | `/gsd-verify-work {N}` |

Phases needing validation: run `/gsd-verify-work {N}` for each flagged phase (the merged eval verdict absorbed the retired `/gsd-validate-phase`).
...
`/gsd-verify-work <N>` for each flagged phase (the merged eval verdict
absorbed the retired `/gsd-validate-phase`), and `/gsd-secure-phase <N>`
...
- [ ] Missing VALIDATION.md phases flagged with `/gsd-verify-work` suggestion (the merged verdict that absorbed `/gsd-validate-phase`)
