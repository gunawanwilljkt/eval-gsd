...
<required_reading>
Read before planning (this is your eval-first context):
- `/Users/gunawanwilljkt/harness/eval-gsd/.claude/get-shit-done/references/eval-first.md` — the eval-first protocol. Always read it; it is a protocol doc, reading it never changes behavior for a phase that has no contract.
- The phase's `{NN}-EVAL-CONTRACT.md` (in the phase directory) **if it exists**. This is the LOCKED, spec-time definition of how the phase is proven. When present, it drives the `emit_acceptance_criteria` step below. When absent, plan exactly as before (backward-compatible — a missing contract is not an error).
</required_reading>
...
@/Users/gunawanwilljkt/harness/eval-gsd/.claude/get-shit-done/references/planner-source-audit.md for full format, examples, and gap-handling rules.
...
@/Users/gunawanwilljkt/harness/eval-gsd/.claude/get-shit-done/references/planner-source-audit.md for constraint examples.
...
**Test:** Could a different Claude instance execute without asking clarifying questions? If not, add specificity. See @/Users/gunawanwilljkt/harness/eval-gsd/.claude/get-shit-done/references/planner-antipatterns.md for vague-vs-specific comparison table.

## TDD Detection

**When `workflow.tdd_mode` is enabled:** Apply TDD heuristics aggressively — all eligible tasks MUST use `type: tdd`. Read @/Users/gunawanwilljkt/harness/eval-gsd/.claude/get-shit-done/references/tdd.md for gate enforcement rules and the end-of-phase review checkpoint format.
...
**When `MVP_MODE` is enabled (passed by the plan-phase orchestrator):** Decompose tasks as **vertical feature slices**, not horizontal layers. Required reading: `@/Users/gunawanwilljkt/harness/eval-gsd/.claude/get-shit-done/references/planner-mvp-mode.md` (loaded conditionally by the orchestrator).
...
1. Frame the phase goal as a user story at the top of `PLAN.md`. The user story is sourced from the `**Goal:**` line in ROADMAP.md (set by `mvp-phase`). Emit it with bolded keywords:
   ...
   **As a** [user role], **I want to** [capability], **so that** [outcome].
   ...
   Format rules from `@/Users/gunawanwilljkt/harness/eval-gsd/.claude/get-shit-done/references/user-story-template.md`:
...
**Walking Skeleton mode** (`WALKING_SKELETON=true`, set by orchestrator for Phase 1 + new project under `--mvp`): The first deliverable is a Walking Skeleton — the thinnest possible end-to-end stack. In addition to `PLAN.md`, produce `SKELETON.md` using the template at `@/Users/gunawanwilljkt/harness/eval-gsd/.claude/get-shit-done/references/skeleton-template.md`. `SKELETON.md` records architectural decisions (framework, DB, auth, deployment, directory layout) that subsequent phases will build on without renegotiating.
...
@/Users/gunawanwilljkt/harness/eval-gsd/.claude/get-shit-done/references/planner-antipatterns.md
...
```bash
node "/Users/gunawanwilljkt/harness/eval-gsd/.claude/get-shit-done/bin/gsd-tools.cjs" graphify status
```
...
```bash
node "/Users/gunawanwilljkt/harness/eval-gsd/.claude/get-shit-done/bin/gsd-tools.cjs" graphify query "<phase-goal-keyword>" --budget 2000
```
...
cat "$phase_dir"/*-EVAL-CONTRACT.md 2>/dev/null   # LOCKED eval contract (eval-first); drives emit_acceptance_criteria
cat "$phase_dir"/*-AI-SPEC.md 2>/dev/null          # AI phases only — §5 eval table is de-orphaned via the same mechanism
```

**If CONTEXT.md exists (has_context=true from init):** Honor user's vision, prioritize essential features, respect boundaries. Locked decisions — do not revisit.

**If `{NN}-EVAL-CONTRACT.md` exists:** Read its `## Rows` table in full and retain it for the `emit_acceptance_criteria` step. Treat the contract as authoritative and already `status: locked` (do NOT author, re-derive, or re-lock it — that is spec-phase's job). Also read `{NN}-AI-SPEC.md` §5 ("Evaluation Strategy") if present; its eval rows are folded into the *same* emit mechanism (de-orphan). If no contract and no AI-SPEC §5 exist, skip the emit step and plan exactly as before.
...
@/Users/gunawanwilljkt/harness/eval-gsd/.claude/get-shit-done/references/thinking-models-planning.md
...
<step name="emit_acceptance_criteria">
## Wire the eval contract into tasks (eval-first)

**Runs only if a `{NN}-EVAL-CONTRACT.md` was read in `gather_phase_context`, OR a `{NN}-AI-SPEC.md` §5 eval table is present.** If neither exists, SKIP this step entirely — the plan is authored exactly as before (backward-compatible). Protocol: `@/Users/gunawanwilljkt/harness/eval-gsd/.claude/get-shit-done/references/eval-first.md`.

The executor already runs `<acceptance_criteria>` as a per-task HARD GATE and is BLOCKED from the next task until they pass (`execute-plan.md` §"HARD GATE — acceptance_criteria verification"). So emitting a contract row's command as a task's `<acceptance_criteria>` makes that eval *gate execution for free*. Your job here is to wire each gate/Code row onto exactly one task.

**Build the eval row set:**
1. From `{NN}-EVAL-CONTRACT.md` `## Rows`, select every row where `severity` is `gate` AND `measurement` is `Code`. (Skip `warn` rows and `Judge`/`Human` rows — those are evaluated at verify-phase, not gated per task. `sample_rate` informs cadence, not emission.)
2. **De-orphan AI-SPEC §5 (AI phases, best-effort):** if `{NN}-AI-SPEC.md` has a §5 "Evaluation Strategy" Dimensions table, scan it for any dimension whose Measurement Approach is **Code** AND that supplies a runnable command (e.g. a CI/CD eval command in §5's "CI/CD Integration", or a Code dimension whose rubric is expressed as an exit-0 command). Add only those to the emit set — they were previously ignored entirely by the planner, which is the orphaning this fixes. Reference each by its dimension name as the row id (e.g. `AISPEC-§5-<dimension>`).
   **Schema caveat (state it, don't fake it):** the §5 Dimensions table (`Dimension | Rubric | Measurement | Priority`) has **no `req_ref`, no per-row `command_or_rubric`, and no `gate`/`warn` severity** — unlike the eval contract. So most §5 rows (LLM-Judge / Human dimensions, or Code dimensions with no concrete command) have nothing runnable to emit and are **carried to verify-phase**, not gated per task. If §5 has gate-worthy Code behaviors but the phase has no `{NN}-EVAL-CONTRACT.md`, do not invent commands here — surface it: note in the plan that this AI phase should have authored an eval contract at spec time (W2's job), and proceed with whatever Code commands §5 does provide. The eval contract supersedes §5's contract role; §5 alone is the legacy fallback.

**Map each selected row to exactly one task and emit it:**
For each row in the set:
1. **Find the owning task** — the single task whose completion makes the row's behavior TRUE (i.e. the task whose `<done>` the row would prove). Match primarily by the row's `req_ref` to the requirement the task implements; if a `req_ref` spans multiple tasks, attach the row to the one task that delivers the behavior the row asserts (the behavior column), not to every task touching that REQ.
2. **Emit the row's `command_or_rubric` verbatim** as one bullet inside that task's `<acceptance_criteria>` XML block. Copy the command character-for-character — do not paraphrase, reformat, wrap in a code fence, or "improve" it. This is **additive**: it is appended alongside the task's existing acceptance_criteria bullets (every task already carries acceptance_criteria per `plan-phase.md`), never replacing them.
3. **Carry the row ids** onto the task as a sibling `<eval_rows>` element listing the contract row ids it satisfies, e.g. `<eval_rows>EC-1</eval_rows>` (comma-separate if a task owns several rows). This is the task↔eval traceability the ledger (W3) and verify-phase (W5) later read.

```xml
<task type="auto">
  <name>Task 1: Implement POST /sessions</name>
  <files>src/api/sessions.ts</files>
  <read_first>...</read_first>           <!-- existing mandatory fields omitted for brevity -->
  <eval_rows>EC-1</eval_rows>            <!-- NEW: contract row ids this task satisfies -->
  <action>...</action>
  <verify>...</verify>
  <acceptance_criteria>
    - {existing task acceptance bullets stay here}
    - {EC-1.command_or_rubric copied verbatim from the contract}   <!-- NEW bullet, additive -->
  </acceptance_criteria>
  <done>...</done>
</task>
```

**Mandatory coverage + uniqueness self-check before finalizing (this is the acceptance):**
- [ ] Every `gate`+`Code` row from the contract (and AI-SPEC §5, if present) is emitted as the `<acceptance_criteria>` of **exactly one** task — none unassigned, none duplicated across tasks.
- [ ] Each emitted command is **byte-identical** to the row's `command_or_rubric`.
- [ ] Each task that satisfies one or more rows carries an `<eval_rows>` element naming those row ids.
- [ ] No `warn`/`Judge`/`Human` row was emitted as a per-task gate (those belong to verify-phase).

If a `gate`+`Code` row has no task that can own it, the plan is incomplete — add the task (do NOT drop the row). Do **not** author, modify, re-derive, or re-lock the contract here, and do **not** run the REQ⇄eval coverage gate against REQUIREMENTS.md — those are spec-phase's responsibility (assume the contract is already locked).
</step>
...
**Carry eval-first wiring into the file:** when `emit_acceptance_criteria` produced any mapping (contract `gate`+`Code` rows, or de-orphaned AI-SPEC §5 Code commands), write each owning task with its `<eval_rows>` element and the row's `command_or_rubric` as an additional `<acceptance_criteria>` bullet (verbatim) — exactly as computed in that step. Do not drop these on write; they are how the eval gates the executor.
...
See @/Users/gunawanwilljkt/harness/eval-gsd/.claude/get-shit-done/references/planner-chunked.md for `## OUTLINE COMPLETE` and `## PLAN COMPLETE` return formats used in chunked mode.
...
- [ ] **Eval-first (if `{NN}-EVAL-CONTRACT.md` or `{NN}-AI-SPEC.md` §5 present):** every contract `gate`+`Code` row is emitted as exactly one task's `<acceptance_criteria>`, verbatim (byte-identical command), with `<eval_rows>` carried on that task; no gate/Code row left unassigned or duplicated. (No contract present → this check is N/A.)
