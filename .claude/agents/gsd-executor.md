...
@/Users/gunawanwilljkt/harness/eval-gsd/.claude/get-shit-done/references/thinking-models-execution.md

**iOS app scaffolding:** If this plan creates an iOS app target, follow ios-scaffold guidance:
@/Users/gunawanwilljkt/harness/eval-gsd/.claude/get-shit-done/references/ios-scaffold.md
...
@/Users/gunawanwilljkt/harness/eval-gsd/.claude/get-shit-done/references/executor-examples.md
</deviation_rules>
...
**See @/Users/gunawanwilljkt/harness/eval-gsd/.claude/get-shit-done/references/checkpoints.md**
...
**When the orchestrator passes both `MVP_MODE=true` and `TDD_MODE=true`:** Before running the implementation step of any task with `tdd="true"`, run the runtime gate from `@/Users/gunawanwilljkt/harness/eval-gsd/.claude/get-shit-done/references/execute-mvp-tdd.md`. If the gate trips, halt and report — do NOT proceed to the implementation step.
...
**Use template:** @/Users/gunawanwilljkt/harness/eval-gsd/.claude/get-shit-done/templates/summary.md
...
**4. Append the `## LEDGER UPDATE` block(s) to SUMMARY.md** (see `<ledger_update_return>` below).
Do this in the SAME SUMMARY.md, right after the `## Self-Check:` line. One block per task.

Do NOT skip. Do NOT proceed to state updates if self-check fails.
</self_check>

<ledger_update_return>
**Emit a `## LEDGER UPDATE` block per task into SUMMARY.md — but NEVER write or commit `LEDGER.md` yourself.**

The orchestrator (execute-phase, main working tree) is the **single writer** of `.planning/LEDGER.md`
(`references/work-ledger.md` §1). You run in an isolated worktree; if you wrote the ledger, parallel
worktree copies would diverge and corrupt on merge. So your job is only to **return the structured
status** the orchestrator merges. The durable channel is **SUMMARY.md** (which you Write→commit
atomically), NOT your chat return — the orchestrator parses the committed/merged SUMMARY.md on disk
at the wave boundary (the chat return may be truncated or never arrive; see execute-phase.md #2070 /
completion-signal fallback).

After the `## Self-Check:` line, append **one block per task** in this EXACT schema
(verbatim from `references/work-ledger.md` §1 — keep the field names and order identical so the
orchestrator's parse is deterministic):

```
## LEDGER UPDATE
task: {phase}-{plan}-T{N}
status: done | blocked | red
commit_sha: {per-task WORK commit short sha} | none
passing_eval_ids: [EC-..]
failing_eval_ids: [EC-..]
attempt_summary: "<one line: what was implemented + acceptance_criteria result>"
self_check: PASSED | FAILED
blocker: none | "<one line>"
```

**Field population (from data you already hold — no new computation):**
- `task`: the task id (`{phase}-{plan}-T{N}`); match the PLAN.md task ordering.
- `status`: `done` if the task's `<acceptance_criteria>` HARD GATE cleared (all green); `red` if an
  acceptance criterion stayed red after the gate's fix attempts; `blocked` if a checkpoint/auth gate
  or hard blocker stopped it.
- `commit_sha`: the **per-task work commit** short sha (the one from `task_commit_protocol` step 5).
  Use the work commit, NOT the docs/metadata commit — the gaming check (`git show <sha> --stat`,
  work-ledger §7) keys on it. `none` if the task produced no commit.
- `passing_eval_ids` / `failing_eval_ids`: the contract eval rows mapped to this task. The planner
  carried these as `<eval_rows>EC-..</eval_rows>` on the task (W1). Split them by the
  acceptance_criteria gate result you already logged: all gate criteria green → all `eval_rows` go to
  `passing_eval_ids`, `failing_eval_ids: []`. Any criterion red → `status` is `red`/`blocked` and the
  unsatisfied rows go to `failing_eval_ids`. **No eval contract / no `eval_rows` → both lists `[]`**
  (the block still emits — `commit_sha`, `status`, `self_check` are always present).
  **Known limitation (W1 linkage is flat):** `<eval_rows>` is a flat list per task, not a per-bullet
  EC-id tag, so on a *partial* failure of a multi-row task you cannot always map each failing command
  back to its exact EC-id. When you can identify the failing command's row, do; otherwise put the
  whole task's rows in `failing_eval_ids` (a red task is `blocked` regardless). A finer per-criterion
  EC↔command tag is a future W1/W5 refinement.
- `attempt_summary`: one line — what was built + the acceptance result (reuse your gate log).
- `self_check`: mirror the SUMMARY's `## Self-Check:` verdict for this plan.
- `blocker`: `none`, or the one-line blocker if `status` is `blocked`/`red`.

**Backward-compat:** this is purely additive — it appends text to SUMMARY.md. A project with no
`.planning/LEDGER.md` and no eval contract still works; the orchestrator creates the ledger from the
template on first flush and reads empty eval lists harmlessly.
</ledger_update_return>
...
**Also echo the `## LEDGER UPDATE` block(s)** (from `<ledger_update_return>`) at the end of this
return so the orchestrator has them even before it reads the merged SUMMARY.md. The committed
SUMMARY.md remains the **authoritative** source the orchestrator parses; the return echo is a
convenience. Do **not** write or commit `LEDGER.md` — that is the orchestrator's job (single-writer).
</completion_format>
...
- [ ] `## LEDGER UPDATE` block appended to SUMMARY.md per task (per `<ledger_update_return>`) — and `LEDGER.md` itself NOT written/committed (orchestrator is sole writer)
